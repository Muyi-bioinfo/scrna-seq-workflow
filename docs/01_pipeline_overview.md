...
| 9+ pseudobulk 确认性 DE（样本级） | `06_pseudobulk_de.R` | `pseudobulk:`（配对 design / min 守卫 / target 白名单；sample_id 由 01 写入） | annotated → `by_celltype/<type>/` 逐类型 DESeq2 结果 + `summary_degs.csv` | `figures/<type>/` volcano/MA/PCA、全局 pseudobulk PCA、05 vs 06 对比条形图 |
